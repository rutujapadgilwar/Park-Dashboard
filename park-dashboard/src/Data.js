...
import allStates from "./Components/NationalParksMap/allStates.json";
...
export const Data = () => {
	...
	useEffect(() => {
		const stateWiseNationalParks = allStates.map((state) => ({
			id: state.id,
			val: state.val,
			nationalParks: parks
				.filter(
		...
		setStateWiseNationalParks(stateWiseNationalParks);
	}, [parks]);

	console.log(stateWiseNationalParks);
};
